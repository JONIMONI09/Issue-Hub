...
> Moved here from JONIMONI09/Mobile-Harness (commit 77d3175) on 2026-09-15: this analysis is tooling/incident documentation, not product documentation - it lives in the Issue Hub now (policy: code repos stay clean).
...
- Follow-up (2026-09-15 evening): a new key was issued and verified with an MCP `initialize` (HTTP 200, server 0.7.2). All data intact.
...
| `POST /api/mcp` with new key (same evening) | HTTP 200 `initialize` - server `vibeworks 0.7.2` |
...
## Related reports
- Upstream issue: https://github.com/MoinMornhart/vibeworks/issues/25 (findings F1-F5 + fix suggestions)
- Local tracking issue: Issue-Hub #1 (status checklist)
- VibeWorks error inbox: 3 reports (incident, analysis, follow-up), all HTTP 202 on 2026-09-15.
